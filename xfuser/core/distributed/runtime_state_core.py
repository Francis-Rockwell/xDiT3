...
class DiTRuntimeStateCore(RuntimeState):
    patch_mode: bool
    ...
    def size_check(self):
        num_sequence_patch = get_sequence_parallel_world_size()
        latents_height = self.input_config.height // self.vae_scale_factor

        if latents_height % num_sequence_patch != 0:
            raise ValueError(
                "The height of the input is not divisible by the number of sequence parallel devices"
            )
        pipeline_patches_height = (latents_height + self.num_pipeline_patch - 1) // self.num_pipeline_patch
        pipeline_patches_height = ((pipeline_patches_height + (num_sequence_patch * self.backbone_patch_size) - 1) // 
                                (self.backbone_patch_size * num_sequence_patch)) * (self.backbone_patch_size * num_sequence_patch)
        ...
        if the_last_pp_patch_height % (self.backbone_patch_size * num_sequence_patch) != 0:
            raise ValueError(
                f"The height of the last pipeline patch is {the_last_pp_patch_height}, "
                f"which is not a multiple of (patch_size * num_sequence_patch): "
                f"{self.backbone_patch_size} * {num_sequence_patch}. Please try to adjust 'num_pipeline_patches "
                f"or sp_degree argument so that the condition are met "
            )
        pipeline_patches_height_list.append(the_last_pp_patch_height)
        return pipeline_patches_height_list

    def generate_pp_sp_patches_data(self):
        num_sequence_patch = get_sequence_parallel_world_size()
        pipeline_patches_height_list = self.size_check()

        flatten_patches_height = [
            pp_patch_height // num_sequence_patch
            for _ in range(num_sequence_patch)
            for pp_patch_height in pipeline_patches_height_list
        ]
        flatten_patches_start_idx = [0] + [
            sum(flatten_patches_height[:i])
        ...
        pp_sp_patches_height = [
            flatten_patches_height[
                pp_patch_idx * num_sequence_patch : (pp_patch_idx + 1) * num_sequence_patch
            ]
            for pp_patch_idx in range(self.num_pipeline_patch)
        ]
        pp_sp_patches_start_idx = [
            flatten_patches_start_idx[
                pp_patch_idx * num_sequence_patch : (pp_patch_idx + 1) * num_sequence_patch + 1
            ]
            for pp_patch_idx in range(self.num_pipeline_patch)
        ]
        
        return pp_sp_patches_height, pp_sp_patches_start_idx
